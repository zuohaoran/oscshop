...
<div class="row">
	<div class="col-xs-12">	
		<form class="form-horizontal"  id="validation-form" method="post" action='<?php echo ($action); ?>'>
			...
			<?php if(!empty($_GET['id'])): ?><input name="order_status_id" type="hidden" value="<?php echo ($_GET['id']); ?>" /><?php endif; ?>
			
			<div class="form-group">
				<label class="col-sm-2 control-label no-padding-left"> * 订单状态名称 </label>
			<div class="col-sm-10">
				<div class="clearfix">
					<input name="name" id="name" class="col-xs-10 col-sm-5"  value="<?php echo ((isset($d["name"]) && ($d["name"] !== ""))?($d["name"]):''); ?>" type="text">
				</div>
			</div>
			</div>
